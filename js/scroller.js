...
(function () {
  ...
  var CoreScroller, Scroller, activatedElement, checkVisibility, doesScroll,
    findScrollableElement, firstScrollableElement, getDimension,
    getScrollingElement, getSign, isScrollableElement, performScroll, root,
    scrollProperties, shouldScroll;
  ...
  shouldScroll = function (element, direction) {
    ...
    if (computedStyle.getPropertyValue("overflow-" + direction) ===
      "hidden") {
      return false;
    }
    if ((ref = computedStyle.getPropertyValue("visibility")) === "hidden" ||
      ref === "collapse") {
      return false;
    }
  ...
  doesScroll = function (element, direction, amount, factor) {
    ...
    delta = getSign(delta);
    return performScroll(element, direction, delta) && performScroll(
      element, direction, -delta);
  };
  isScrollableElement = function (element, direction, amount, factor) {
    ...
    return doesScroll(element, direction, amount, factor) && shouldScroll(
      element, direction);
  };
  findScrollableElement = function (element, direction, amount, factor) {
    var ref;
    while (element !== getScrollingElement() && !isScrollableElement(
        element, direction, amount, factor)) {
      element = (ref = DomUtils.getContainingElement(element)) != null ?
        ref : getScrollingElement();
    }
    return element;
  };
  ...
  checkVisibility = function (element) {
    ...
    if (rect.bottom < 0 || rect.top > window.innerHeight || rect.right < 0 ||
  ...
  CoreScroller = {
    ...
    scroll: function (element, direction, amount, continuous) {
      var activationTime, animate, calibration, duration,
        myKeyIsStillDown, previousTimestamp, ref, sign, totalDelta,
        totalElapsed;
      if (continuous == null) {
        continuous = true;
      }
      ...
      animate = (function (_this) {
        return function (timestamp) {
          ...
          if (myKeyIsStillDown() &&
            75 <= totalElapsed &&
            (_this.minCalibration <= calibration && calibration <=
              _this.maxCalibration)) {
            if (1.05 * calibration * amount < _this.calibrationBoundary) {
              calibration *= 1.05;
            }
            if (_this.calibrationBoundary < 0.95 * calibration *
              amount) {
              calibration *= 0.95;
            }
          }
          delta = Math.ceil(amount * (elapsed / duration) *
            calibration);
          delta = myKeyIsStillDown() ? delta : Math.max(0, Math.min(
            delta, amount - totalDelta));
          if (delta && performScroll(element, direction, sign *
              delta)) {
            totalDelta += delta;
            return requestAnimationFrame(animate);
          } else {
  ...
  Scroller = {
    ...
    scrollBy: function (direction, amount, factor, continuous) {
      ...
      activatedElement || (activatedElement = (getScrollingElement() &&
        firstScrollableElement()) || getScrollingElement());
      if (!activatedElement) {
        return;
      }
      if (!CoreScroller.wouldNotInitiateScroll()) {
        element = findScrollableElement(activatedElement, direction,
          amount, factor);
    ...
    scrollTo: function (direction, pos) {
      ...
      amount = getDimension(element, direction, pos) - element[
        scrollProperties[direction].axisName];
    ...
    scrollIntoView: function (element) {
      ...
      if (rect != null) {
        if (rect.bottom < 0) {
          ...
          amount = rect.top + Math.min(rect.height - window.innerHeight,
            0);
          element = findScrollableElement(element, "y", amount, 1);
          CoreScroller.scroll(element, "y", amount, false);
        }
